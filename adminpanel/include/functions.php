...
function checkUser()
{
	...
	if (!isset($_SESSION['user_id'])) {
		header('Location: ' . WEB_ROOT . 'adminpanel/login.php');
		exit;
	}
...
{	
	...
	$userName = mysqli_real_escape_string($link, $_POST['txtUserNameAdmin']);
	$password = mysqli_real_escape_string($link, $_POST['txtPasswordAdmin']);		
	...
	if ($userName == '') {
		...
		if ($stmt->rowCount() == 1) {
			...
			if (isset($_SESSION['login_return_url'])) {
				header('Location: ' . WEB_ROOT . 'adminpanel/index.php');
				exit;
			} else {
				header('Location: ' . WEB_ROOT . 'adminpanel/index.php');
				exit;
...
function doLogout()
{
	...
	header('Location: Location: ' . WEB_ROOT . 'adminpanel/login.php');
	exit;
}
