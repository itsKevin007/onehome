...
/*
	...
	required in main and admin pages
*/
...
/* Featured News Paging */

function getPagingLink1($sql, $itemPerPage = 10, $strGet = '')
{
	$result        = dbQuery($sql);
	$pagingLink    = '';
	$totalResults  = dbNumRows($result);
	...
	if ($totalPages > 1) {
		...
		for($page = $start; $page <= $end; $page++)	{
			if ($page == $pageNumber) {
				...
				if ($page == 1) {
					$pagingLink[] = " <a href=\"$self?$strGet\"><font color='#99cc66'>$page</font></a> ";
				} else {
					$pagingLink[] = " <a href=\"$self?page=$page&$strGet\"><font color='#99cc66'>$page</font></a> ";
				}
			}
